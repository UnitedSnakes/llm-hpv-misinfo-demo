...
assert check_completion_format(df)[0].shape[0] == 0
...
print("Unfiltered inference results (contain 'not inferred'):")
for element, count in zip(unique_elements, counts):
    print(f"{Config.patterns[element - 1][0]}: {count} times")
print("\n")

# stats about rows that were "not inferred"
df_not_inferred = df[df["inference_results"].str.startswith("not inferred")]
print("not inferred prompts:")
print(df_not_inferred["prompt_type"].value_counts())
print("\n")

# filter out rows that were not inferred
# print(df_not_inferred.head(26))
# print(df_not_inferred.shape)
# print(df.head(26))
# print(df.shape)
# df -= df_not_inferred
...
for i in range(Config.test_size):
    ...
    print(
        f"Filtered {np.count_nonzero(mask == 0)} rows that were not inferred under {Config.prompt_version_list[i]}."
    )
    ...
    df_report = pd.DataFrame(
        classification_report(
            ...
            labels=[1, 2, 3],
            target_names=[label for label in [synnonym[0] for synnonym in Config.patterns][:3]],
            output_dict=True,
        )
    )
    ...
    df_cm = pd.DataFrame(
        ...
        index=[
            f"True {label}"
            for label in [synnonym[0] for synnonym in Config.patterns][:3]
        ],
        columns=[
            f"Inferred {label}"
            for label in [synnonym[0] for synnonym in Config.patterns][:3]
    ...
    print(df_cm)
    print(f"Class labels: [0, 1, 2] = {[label for label in [synnonym[0] for synnonym in Config.patterns][:3]]}")
